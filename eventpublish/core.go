...
import (
	...
	epCom "AppPlaygroundService/eventpublish/common"
	"AppPlaygroundService/eventpublish/grpc"
	"AppPlaygroundService/eventpublish/native"
	...
	for _, plugin := range mviper.Get("event_publish.plugins").([]interface{}) {
		...
		if t, ok := configMap["type"].(string); ok {
			...
			case epCom.GRPCType:
				...
				startPluginInput.StartCmdArgs = interface2map(configMap["start_cmd_args"])
