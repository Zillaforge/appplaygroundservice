...
func NewAccessParams() *AccessFormatterParams {
	...
	p.Meta.AvailabilityDistrict = mviper.GetString("app_playground_service.scopes.availability_district")
	return p
}

func InitAccessLogger(fileName string) {
	_accessLogger = &lumberjack.Logger{
		Filename:   filepath.Join(mviper.GetString("app_playground_service.logger.access_log.path"), fileName),
		MaxSize:    mviper.GetInt("app_playground_service.logger.access_log.max_size"),    // MB
		MaxBackups: mviper.GetInt("app_playground_service.logger.access_log.max_backups"), // how many backup for log file.
		MaxAge:     mviper.GetInt("app_playground_service.logger.access_log.max_age"),
		Compress:   mviper.GetBool("app_playground_service.logger.access_log.compress"),
	}
}
