...
func (m *Method) DeleteModuleCategory(ctx context.Context, input *pb.DeleteInput) (output *pb.DeleteOutput, err error) {
	var (
	...
	whereInput := storCom.DeleteModuleCategoryWhere{}
	...
	deleteInput := &storCom.DeleteModuleCategoryInput{
		Where: whereInput,
	}

	deleteOutput, err := storages.Use().DeleteModuleCategory(ctx, deleteInput)
	if err != nil {
		if e, ok := tkErr.IsError(err); ok {
			switch e.Code() {
			case cnt.StorageModuleCategoryInUseErrCode:
				err = tkErr.New(cCnt.GRPCModuleCategoryInUseErr)
				return
			}
		}
		zap.L().With(
			zap.String(cnt.GRPC, "storages.Use().DeleteModuleCategory()"),
			zap.String(cnt.RequestID, requestID),
