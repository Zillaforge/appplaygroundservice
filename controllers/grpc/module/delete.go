...
func (m *Method) DeleteModule(ctx context.Context, input *pb.DeleteInput) (output *pb.DeleteOutput, err error) {
	var (
	...
	whereInput := storCom.DeleteModuleWhere{}
	...
	deleteInput := &storCom.DeleteModuleInput{
		Where: whereInput,
	}

	deleteOutput, err := storages.Use().DeleteModule(ctx, deleteInput)
	if err != nil {
		if e, ok := tkErr.IsError(err); ok {
			switch e.Code() {
			case cnt.StorageModuleInUseErrCode:
				err = tkErr.New(cCnt.GRPCModuleInUseErr)
				return
			}
		}
		zap.L().With(
			zap.String(cnt.GRPC, "storages.Use().DeleteModule()"),
			zap.String(cnt.RequestID, requestID),
