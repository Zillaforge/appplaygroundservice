...
// CreateModule ...
func (o *Operation) CreateModule(ctx context.Context, input *common.CreateModuleInput) (output *common.CreateModuleOutput, err error) {
	var (
	...
	if createErr := o.conn.WithContext(ctx).Clauses(clause.Returning{}).Create(&input.Module).Error; createErr != nil {
		if sqlErr, ok := createErr.(*mysql.MySQLError); ok {
			...
			case mariadb_com.ER_DUP_ENTRY:
				...
				err = tkErr.New(cnt.StorageModuleExistErr).WithInner(createErr)
				return
			}
		}
		zap.L().With(
	...
	output = &common.CreateModuleOutput{
		Module: input.Module,
	}
	return
}
